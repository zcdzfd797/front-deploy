...
- **按目标打包**：点击「打包」先选择目标（可多选），每个目标使用自己的构建命令依次构建并生成专属压缩包（`{产物目录}_{目标ID}.zip`）
- 编辑项目可同时维护「默认构建命令」与「按目标构建命令」（留空的目标使用默认命令）——不同部署位置需要不同构建产物时按目标覆盖
- 打包前执行 Git 一致性校验（远程/本地）与分支校验
- 构建命令实时输出日志（SSE），自动识别构建产物目录并压缩
- 部署时先弹出目标多选（默认全选已打包目标，未打包目标自动禁用），确认后按顺序逐个目标部署——每个目标上传自己的专属压缩包
- 单个目标失败不影响后续目标，结束汇总“成功/失败”数量
- 项目卡片上的目标标签可点击单发部署到该目标；每个目标独立记录打包时间、部署时间与状态（支持“部分部署”状态）
- 部署时上传 zip 到目标服务器，自动备份同名目录（时间戳）并解压新包
- 备份管理支持按目标操作：查看备份列表、逐项一键回滚（当前线上目录先转存为新备份，所选备份保留可重复回滚）、勾选批量删除
- 旧版数据自动迁移：已有的单压缩包记录会平摊到当前关联的目标上
...
- 主区：全局控制与项目工作区（分组、筛选、服务器管理入口、项目卡片）独占全宽
- 操作终端为**悬浮窗**：可拖拽（标题栏）、可缩放（右下角）、可折叠/隐藏（顶部「操作终端」按钮开关），位置与尺寸自动记忆；执行打包/部署/回滚等操作时自动唤起
- 顶部精简，仅保留日期信息与终端开关
